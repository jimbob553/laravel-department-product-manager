...
@section('content')
  <div class="max-w-2xl mx-auto bg-white shadow-md rounded-xl p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold text-purple-700">Add New Department</h1>
    ...
    <form method="POST" action="{{ route('departments.store') }}" class="space-y-4">
      ...
      <div>
        ...
        <textarea id="description" name="description" rows="3"
                  class="mt-1 border rounded-md p-2 w-full focus:ring-purple-400 focus:border-purple-400">{{ old('description') }}</textarea>
        @error('description') 
          <p class="text-red-600 text-sm mt-1">{{ $message }}</p> 
        @enderror
      </div>
